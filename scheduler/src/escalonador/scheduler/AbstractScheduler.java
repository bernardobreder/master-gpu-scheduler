...
public abstract class AbstractScheduler {
	...
	protected RoundKernel initialKernels;

	protected List<RoundKernel> roundsStep;

	protected final LinkedList<Integer> streams = new LinkedList<>();

	protected int numberOfStream;

	protected List<Kernel> preparedKernels;

	public abstract void execute() throws IOException;

	protected List<Kernel> prepareKernels(ResourceSet resourceSet, List<Kernel> kernelList) {
		return new ArrayList<>(kernelList);
	}
	...
	public List<Resource> processResources(Resource kernelLimit, List<List<Entry<Integer, Kernel>>> rounds) {
		...
		for (int i = 0; i < rounds.size(); i++) {
			...
			for (Entry<Integer, Kernel> entry : rounds.get(i)) {
				...
				resource.threads = kernel.getThread();
			}
		...
		try (FileWriter file = new FileWriter("dynamic-serial-schedule-rounds-2.txt")) {
			...
			for (int n = 0; n < spCount && !kernels.isEmpty(); n++) {
				...
				for (Kernel kernel : round) {
					...
					threadHas -= kernel.getThread();
					threadUsing += kernel.getThread();
				}
			...
			for (int reg = kernelUnit.register; reg <= kernelGlobal.register; reg += kernelUnit.register) {
				for (int sm = kernelUnit.sharedMemory; sm <= kernelGlobal.sharedMemory; sm += kernelUnit.sharedMemory) {
					for (int th = kernelUnit.threads; th <= kernelGlobal.threads; th += kernelUnit.threads) {
						...
						int thDelta = th - kernel.getThread();
	...
	public List<RoundKernel> getRoundsStep() {
		return roundsStep;
	}

	public RoundKernel getInitialKernels() {
	...
	public List<Kernel> getPreparedKernels() {
		return preparedKernels;
	}
}
